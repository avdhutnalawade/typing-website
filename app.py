...
<div class="modal" id="createBox" onclick="event.stopPropagation()">
    ...
    <input type="text" id="newUsername" placeholder="Username (Min 8 Letters)">
...
function filterAdminData(days) {
    ...
    Object.keys(globalUserStats).forEach(name => {
        ...
        if (!user.history || days === 'all') { 
            processedStats[name] = { attempts: user.attempts, bestWpm: user.bestWpm, accuracy: user.accuracy }; 
            return; 
        }
...
async function finishTest(){
    clearInterval(timer); isPaused = true; testRunning = false; 
