...
HTML_CODE = """
<!DOCTYPE html><html><head><title>Beginner Typing Speed Test</title>
...
.correct {color:#00ff88;} .wrong {color:#ff4d4d;}
...
<div id="finishOverlay">
    <h1>Congratulations! 🚀</h1><div id="finalScore"></div>
    <div style="display:flex; gap:10px;"><button class="btn" onclick="nextLevel()">Next Level</button><button class="btn" style="background:gray;" onclick="restartTest()">Restart</button></div>
</div>
<div class="left-menu">
<button onclick="showTest()">Test</button><button onclick="openLogin()">Login</button><button onclick="openCreate()">Create Account</button>
<button onclick="openAdmin()" style="background:#ff4d4d;">ADMIN PANEL</button><button onclick="openSettings()" style="background:#ffa500;">SETTINGS ⚙️</button><button onclick="openLearn()" style="background:#28a745;">LEARN 🎓</button>
</div>
<div class="modal" id="learnBox" onclick="event.stopPropagation()">
    <h2 style="color: #28a745">Learn Typing</h2>
    <div class="step-item"><h3>Step 1: Proper Positioning</h3><iframe class="video-frame" src="https://www.youtube.com/embed/1ArVtCQqQRE" frameborder="0" allowfullscreen></iframe></div>
    <div class="step-item"><h3>Step 2: Pro Techniques</h3><iframe class="video-frame" src="https://www.youtube.com/embed/EMUH9BPmqcY" frameborder="0" allowfullscreen></iframe></div>
    <button class="btn" onclick="closeAll()">Close</button>
</div>
<div class="modal" id="settingsBox" onclick="event.stopPropagation()">
    <h2>Settings</h2><label>Theme:</label>
    <select id="themeSelect" onchange="changeTheme()"><option value="default">Midnight Dark</option><option value="light-theme">Arctic White</option><option value="neon-theme">Neon Glow</option><option value="matrix-theme">The Matrix</option></select>
    <label>Audio:</label><select id="audioSelect"><option value="classic">Classic Square</option><option value="mechanical">Mechanical Keyboard</option><option value="silent">No Sound</option></select>
    <br><br><button class="btn" onclick="closeAll()">Save & Close</button>
</div>
<div class="modal" id="loginBox" onclick="event.stopPropagation()">
<h3>Login</h3><input type="text" id="username" placeholder="Username"><input type="password" id="password" placeholder="Password"><button class="btn" onclick="login()">Login</button><button class="btn" style="background:gray" onclick="closeAll()">Close</button>
</div>
<div class="modal" id="createBox" onclick="event.stopPropagation()">
<h3>Create Account</h3><input type="text" id="newUsername" placeholder="Username (Min 8 Letters)"><input type="password" id="newPassword" placeholder="Strong Password"><button class="btn" onclick="createAccount()">Create</button><button class="btn" style="background:gray" onclick="closeAll()">Close</button>
</div>
<div class="modal" id="adminPanel" onclick="event.stopPropagation()">
    ...
    <div id="statsTable"></div><br><button class="btn" onclick="closeAll()">Close Dashboard</button>
</div>
<div id="welcomeScreen"><div id="welcomeText">Welcome 🚀</div><button class="start-btn" onclick="startSite()">Start</button></div>
<div class="header"><h2>Typing Speed Test</h2><div id="userDisplay"></div></div>
<div class="center"><div id="timer"></div><div id="task"></div><input id="hiddenInput"><div id="result"></div>
<div id="gameButtons" style="display:none;"><button class="btn" onclick="nextLevel()">Next Level</button><button class="btn" onclick="restartTest()" style="background:gray;">Restart</button></div>
</div>
...
<h2>Select Time</h2><button onclick="startTest(60)">1 Min</button><button onclick="startTest(180)">3 Min</button><button onclick="startTest(300)">5 Min</button><button onclick="startTest(600)">10 Min</button>
</div>
<script>
let currentUser = null; let currentLevelIndex = 0; let isPaused = false; let testRunning = false; let audioCtx; let timer, timeLeft=60; let startTime,totalTyped=0; let currentText = ""; let globalUserStats = {};
const typingLevels = [
...
function closeAll(){ loginBox.style.display="none"; createBox.style.display="none"; settingsBox.style.display="none"; learnBox.style.display="none"; if(document.getElementById("testBox")) document.getElementById("testBox").style.display="none"; adminPanel.style.display="none"; finishOverlay.style.display="none"; }
...
async function login(){
    ...
    if(res.ok){ currentUser = u; document.getElementById("userDisplay").innerHTML = "<span class='user-circle'>👤 "+u+"</span> <button onclick='logout()' style='background:none; border:1px solid #ff4d4d; color:#ff4d4d; border-radius:5px; margin-left:10px; cursor:pointer;'>Logout</button>"; alert("Login Successful!"); closeAll(); } else{ alert("Wrong Username or Password"); }
}
...
async function createAccount(){
    ...
    // 1. Username Length Check (Minimum 8 Characters)
    if(u.length < 8){
    ...
    // Needs: Min 8 chars, 1 Uppercase (A-Z), 1 Number (0-9), 1 Special Character (!@#$%^&*)
    ...
    if(u && p){
        ...
        if(res.ok){ alert("Account Created for " + u); closeAll(); } else { alert("User already exists or error!"); }
    } else{ alert("Enter Username & Password"); }
}

function logout(){ currentUser = null; document.getElementById("userDisplay").innerHTML = ""; }
async function openAdmin(){ stopTyping(); closeAll(); document.getElementById("adminPanel").style.display = "block"; const res = await fetch('/api/admin_data'); globalUserStats = await res.json(); filterAdminData('all'); }
...
function filterAdminData(days) {
    ...
    let processedStats = {}; let now = new Date();
    Object.keys(globalUserStats).forEach(name => {
        let user = globalUserStats[name];
        if (!user.history || days === 'all') { processedStats[name] = { attempts: user.attempts, bestWpm: user.bestWpm, accuracy: user.accuracy }; return; }
        let filteredAttempts = 0; let filteredBestWpm = 0; let filteredAcc = 0;
        user.history.forEach(entry => {
            let entryDate = new Date(entry.date_time.replace(' ', 'T'));
            if ((now - entryDate) / (1000 * 60 * 60 * 24) <= days) {
                filteredAttempts++; if (entry.wpm > filteredBestWpm) filteredBestWpm = entry.wpm; filteredAcc = entry.accuracy;
            }
        });
        processedStats[name] = { attempts: filteredAttempts, bestWpm: filteredBestWpm, accuracy: filteredAcc };
    });
    ...
    sortedUsers.forEach((name, index) => { let s = processedStats[name]; html += `<tr><td>#${index + 1}</td><td>${name}</td><td>${s.attempts}</td><td>${s.bestWpm}</td><td>${s.accuracy}%</td></tr>`; });
    html += "</table>"; document.getElementById("statsTable").innerHTML = html;
}
function stopTyping(){ clearInterval(timer); isPaused = true; document.getElementById("hiddenInput").blur(); }
function resumeTyping(){ if(isPaused && timeLeft > 0 && testRunning){ isPaused = false; clearInterval(timer); timer = setInterval(updateTimer, 1000); document.getElementById("hiddenInput").focus(); } }
window.addEventListener("keydown", function(e) { if(e.code === "Space" && isPaused && finishOverlay.style.display !== "flex" && testRunning) { resumeTyping(); e.preventDefault(); } if(e.code === "Enter" && testRunning) { finishTest(); } });
function unlockAudio(){ if(!audioCtx) audioCtx = new (window.AudioContext || window.webkitAudioContext)(); }
function playKeySound(){
    let type = document.getElementById("audioSelect").value; if(type === "silent" || !audioCtx) return;
    let osc = audioCtx.createOscillator(); let gain = audioCtx.createGain();
    if(type === "mechanical") { osc.type = "sawtooth"; osc.frequency.value = 150 + Math.random()*50; gain.gain.value = 0.02; }
    else { osc.type = "square"; osc.frequency.value = 200 + Math.random()*100; gain.gain.value = 0.05; }
    osc.connect(gain); gain.connect(audioCtx.destination); osc.start(); osc.stop(audioCtx.currentTime + 0.05);
}
function loadText(){ closeAll(); result.innerHTML=""; document.getElementById("gameButtons").style.display="none"; currentText = typingLevels[currentLevelIndex % typingLevels.length]; let html=""; for(let i=0;i<currentText.length;i++){ html+="<span>"+currentText[i]+"</span>"; } task.innerHTML=html; hiddenInput.value=""; startTime = new Date().getTime(); isPaused = false; testRunning = true; clearInterval(timer); timer = setInterval(updateTimer, 1000); }
...
hiddenInput.addEventListener("input",function(){
    if(isPaused || !testRunning) return; playKeySound(); let input=this.value; let spans=document.querySelectorAll("#task span"); totalTyped=input.length;
    for(let i=0;i<spans.length;i++){ if(input[i]==null) spans[i].classList.remove("correct","wrong"); else if(input[i]===currentText[i]){ spans[i].classList.add("correct"); spans[i].classList.remove("wrong"); } else { spans[i].classList.add("wrong"); spans[i].classList.remove("correct"); } }
    if(input===currentText) finishTest();
});
async function finishTest(){
    clearInterval(timer); isPaused = true; testRunning = false; let timeUsed = (new Date().getTime() - startTime) / 60000; let wpm = Math.round((totalTyped/5)/timeUsed) || 0; let correctChars = document.querySelectorAll(".correct").length; let acc = totalTyped > 0 ? Math.round((correctChars / totalTyped) * 100) : 0;
    document.getElementById("finalScore").innerText = "Speed: " + wpm + " WPM | Accuracy: " + acc + "%"; document.getElementById("finishOverlay").style.display = "flex";
